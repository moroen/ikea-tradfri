...
class DeviceView(web.View):
    async def get(self):
        device = await Devices.get_device(self.request.app['api'],
    ...
    async def put(self):
        if self.request.body_exists:
            return web.json_response(
                await Server_Commands.serverCommand(self.request))
        else:
            return web.json_response(Server_Commands.return_object(
